...
_sys.path.append(os.path.join(ROOT, "data", "intelligence"))
try:
    from active_strategies import get_combined_signal
except ImportError:
    def get_combined_signal(df): return "HOLD", "Strategy module missing"
    ...
    while True:
        try:
            ...
            for ticker in TICKERS:
                ...
                # ██ [v5.1] DANGER 상태이면 전체 매수 스킵 (안전장치) ██
                if left_brain.should_skip_buy():
                    log(f"[LEFT BRAIN 차단] {ticker} 시장 위험 신호 - 매수 전체 스킵", alert=False)
                    continue

                # ██ [v5.5] Dynamic Strategy Fusion (Top 3 Strategies) ██
                df_ohlcv = pyupbit.get_ohlcv(ticker, interval="minute15", count=100) # 더 정확한 분석을 위해 15분봉 사용
                strat_signal, strat_reason = get_combined_signal(df_ohlcv)

                log(f"[SCAN] {ticker} | P={ind['price']:,.0f} | RSI={ind['rsi']} | ML={ml_prob*100:.1f}% | Strategy={strat_signal}")
                
                # 3. 진입 로직 (전략 다수결 + ML 승인)
                if strat_signal == "BUY" and ml_prob >= adj_threshold:
                    strategy_type = strat_reason
                    # 포트폴리오 제한량 체크
