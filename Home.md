...
## 🗄 외부 참고자료 (이 저장소 밖, 로컬 PC 전용)

이 저장소를 git으로 다루지 않는 참고자료 보관 위치. 클라우드/샌드박스 세션에서는 접근 불가 — 로컬 PC 세션에서만 확인.

| 위치 | 내용 |
|---|---|
| `D:\에이전트자료저장고` | GitHub 저자별로 모은 순수 참고자료(코드 스냅샷, 페이퍼 리스트). `.git` 없음, 이 저장소와 이름 중복 없음 — 그대로 참고용 |
| `E:\안티그라비티 자료` | 마찬가지로 대부분 GitHub 참고자료 스냅샷. **단, 그 안의 `brain/`이 바로 이 저장소(`nowhere78/home`)의 실제 로컬 작업 폴더이고, `알파에이전트/`는 별도 저장소(`Nowhere78/apar`)의 작업 폴더임** — 둘 다 `.git` 있는 live 저장소이니 그 안의 파일은 참고자료로 취급하지 말 것 |

> ⚠️ 2026-06-22 발견: `E:\안티그라비티 자료\brain` 안의 세션 로그(`*/​.system_generated/logs/transcript.jsonl`)에 OpenAI API 키가 평문으로 커밋된 적이 있어 GitHub push protection에 걸렸음. 히스토리에서 제거하고 push 완료. 세션 로그 자동 동기화 시 시크릿이 섞여 들어갈 수 있으니 주기적으로 점검할 것.
