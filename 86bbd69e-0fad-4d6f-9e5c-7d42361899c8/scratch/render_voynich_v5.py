...
TEXT_BLOCKS = [
    ...
    ("그들 모두가 이 문서 앞에서\n무릎을 꿇었습니다.", 4.5), # Manual break for the problematic line
    ("그런데 2024년, AI가 마침내 해독에 성공했습니다.", 4.5),
    ("하지만... 그 결과를 본 과학자들은 침묵했습니다.", 4.5),
    ("히브리어 기반의 고도화된 암호.", 3.5),
    ("그 속엔 지구 어디에도 존재하지 않는\n기이한 식물들의 해부도가 담겨 있었죠.", 6.5),
    ("생물학자들은 말합니다.\n이건 우리가 아는 생명의 구조가 아니라고.", 5.5),
    ("대체 누구를 위해,\n무엇을 보고 그린 걸까요?", 4.5),
    ("AI는 길을 찾았지만,\n우리는 새로운 질문을 마주했습니다.", 5.5),
    ("이 책, 정말 지구인이 쓴 게 맞을까요?", 4.5),
    ("당신의 생각을 댓글로 남겨주세요.", 3.5),
    ("다음 진실은 여러분의 답변에서 시작됩니다.", 4.5)
]

FULL_TEXT = " ".join([t[0].replace("\n", " ") for t in TEXT_BLOCKS])
VOICE = "ko-KR-InJoonNeural" 
RATE = "+10%" 
OUTPUT_AUDIO = "voynich_audio_v5.mp3"
...
OUTPUT_VIDEO = "voynich_shorts_v5_pro.mp4"

# Use / instead of \ for FFmpeg compatibility
IMAGE_HOOK = IMAGE_HOOK.replace("\\", "/")
IMAGE_PLANT = IMAGE_PLANT.replace("\\", "/")
...
def render_video_with_subtitles():
    ...
    for text, duration in TEXT_BLOCKS:
        ...
        # Style: Slightly smaller font (60), center, Malgun Gothic
        f = (f"drawtext=fontfile='{FONT_PATH}':text='{clean_text}':fontcolor=yellow:fontsize=65:x=(w-text_w)/2:y=(h-text_h)/2"
             f":line_spacing=10:borderw=4:bordercolor=black:enable='between(t,{current_time},{current_time + duration})'")
...
async def main():
    ...
    print("Rendering Professional Video V5 (Multi-line subtitles)...")
    render_video_with_subtitles()
    if os.path.exists(OUTPUT_VIDEO):
        print(f"Success! V5 Video saved as {OUTPUT_VIDEO}")
