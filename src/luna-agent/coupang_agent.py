...
def log(msg):
    ...
    print(f"[{ts}] [CoupangAgent] {msg}", flush=True)
...
if __name__ == "__main__":
    ...
    # nargs='+'를 추가하여 여러 상품명을 한꺼번에 받을 수 있게 수정
    parser.add_argument("--product", type=str, nargs='+', help="수동 지정 상품명들 (없으면 AI가 트렌드 기반 자동 선택)")
    parser.add_argument("--category", type=str, default="트렌드", help="상품 카테고리")
    
    args = parser.parse_args()
    
    agent = CoupangAgent()
    log("🚀 Alpha Coupang Pro 에이전트 가동 시작...")
    
    # 상품명이 있으면 수동 모드, 없으면 자동(트렌드) 모드로 실행
